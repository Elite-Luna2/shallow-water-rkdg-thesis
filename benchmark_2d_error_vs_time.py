...
def main():
    ...
    df = pd.read_csv("/home/c_a00/SWE_flooding_project/swashes_2d_thacker_radial.csv")
    ...
    def initial_hu(x):
        values = np.empty((1, x.shape[1]), dtype=PETSc.ScalarType)
        h_vals = interp2d(x[0], x[1], h_ref)
        u_vals = interp2d(x[0], x[1], u_ref)
        values[0] = h_vals * u_vals
        return values

    def initial_hv(x):
        values = np.empty((1, x.shape[1]), dtype=PETSc.ScalarType)
        h_vals = interp2d(x[0], x[1], h_ref)
        v_vals = interp2d(x[0], x[1], v_ref)
        values[0] = h_vals * v_vals
        return values
    ...
    if comm.rank == 0:
        ...
        print(
            f"step={0:05d} t={0.0:.6f} "
            f"L1={l1:.6e} L2={l2:.6e} Linf={linf:.6e}"
        )

    t = 0.0
    step = 0
    h_min = min((xmax - xmin) / nx, (ymax - ymin) / ny)
    ...
    if comm.rank == 0:
        ...
        plt.xlabel("Time [s]")
        plt.ylabel("Error")
        plt.title("2D Thacker Error Norms vs Time")
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
        plt.savefig("output/swashes_2d_thacker_error_vs_time.png", dpi=200)
        plt.show()
